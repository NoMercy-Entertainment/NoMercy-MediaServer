...
export default function(req: Request, res: Response) {
	...
	const result = {
		...
		track: music.musicGenre_track.map((t) => {
			...
			const artists = artistResult.filter(a => a.track_id == t.track_id)
				.map(a => ({
					id: a?.artist?.id,
					name: a?.artist?.name,
					...
					origin: deviceId,
					color_palette: a?.artist?.colorPalette,
				}));
			const albums = albumResult.filter(a => a.track_id == t.track_id)
				.map(a => ({
					id: a?.album?.id,
					name: a?.album?.name,
					...
					origin: deviceId,
					color_palette: a?.album?.colorPalette,
				}));
			...
			return {
				...
				color_palette: JSON.parse((albums[0] ?? t).colorPalette ?? '{}'),
