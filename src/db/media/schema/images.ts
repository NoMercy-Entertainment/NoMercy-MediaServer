...
export const images = sqliteTable('images', {
	...
	index: index('images_index').on(db.episode_id, db.movie_id, db.person_id, db.season_id, db.tv_id, db.album_id, db.artist_id, db.cast_id, db.crew_id, db.collection_id, db.track_id),
	unique: uniqueIndex('images_inique').on(db.filePath),
	unique_tv: uniqueIndex('images_tv_unique').on(db.tv_id, db.filePath),
	unique_season: uniqueIndex('images_season_unique').on(db.season_id, db.filePath),
	unique_episode: uniqueIndex('images_episode_unique').on(db.episode_id, db.filePath),
	unique_movie: uniqueIndex('images_movie_unique').on(db.movie_id, db.filePath),
	unique_collection: uniqueIndex('images_collection_unique').on(db.collection_id, db.filePath),
	unique_person: uniqueIndex('images_person_unique').on(db.person_id, db.filePath),
	unique_cast: uniqueIndex('images_cast_unique').on(db.cast_id, db.filePath),
	unique_crew: uniqueIndex('images_crew_unique').on(db.crew_id, db.filePath),
	unique_artist: uniqueIndex('images_artist_unique').on(db.artist_id, db.filePath),
	unique_album: uniqueIndex('images_album_unique').on(db.album_id, db.filePath),
	unique_track: uniqueIndex('images_track_unique').on(db.track_id, db.filePath),
}));
