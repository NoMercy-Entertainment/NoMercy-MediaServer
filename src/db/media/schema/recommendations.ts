...
export const recommendations = sqliteTable('recommendations', {
	...
	pk: primaryKey(db.id),
	index: index('recommendations_index').on(db.id, db.media_id, db.tvFrom_id, db.tvTo_id, db.movieFrom_id, db.movieTo_id),
	tvUnique: uniqueIndex('recommendations_tv_unique').on(db.media_id, db.tvFrom_id),
	movieUnique: uniqueIndex('recommendations_movie_unique').on(db.media_id, db.movieFrom_id),
}));
