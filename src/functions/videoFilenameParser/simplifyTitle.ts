...
const simpleTitleRegex
  = /\s*(?:480[ip]|576[ip]|720[ip]|1080[ip]|2160[ip]|HVEC|[xh][\W_]?26[45]|DD\W?5\W1|[<>?*:|]|848x480|1280x720|1920x1080)((8|10)b(it))?/iu;
const websitePrefixRegex = /^\[\s*[a-z]+(\.[a-z]+)+\s*\][- ]*|^www\.[a-z]+\.(?:com|net)[ -]*/iu;
const cleanTorrentPrefixRegex = /^\[(?:REQ)\]/iu;
const cleanTorrentSuffixRegex = /\[(?:ettv|rartv|rarbg|cttv)\]$/iu;

/** Used to help cleanup releases that often emit the year title.SCR-group */
const commonSourcesRegex
  = /\b(Bluray|(dvdr?|BD)rip|HDTV|HDRip|TS|R5|CAM|SCR|(WEB|DVD)?.?SCREENER|DiVX|xvid|web-?dl)\b/iu;
...
export function simplifyTitle(title: string): string {
	...
	simpleTitle = simpleTitle.replace(new RegExp(commonSourcesRegex, 'igu'), '');
...
export function releaseTitleCleaner(title: string): string | null {
	...
	trimmedTitle = trimmedTitle.replace(new RegExp(commonSourcesRegex, 'igu'), '').trim();
	...
	trimmedTitle = trimmedTitle.replace(new RegExp(sceneGarbageExp, 'igu'), '').trim();

	for (const lang of Object.values(Language)) {
		trimmedTitle = trimmedTitle.replace(new RegExp(`\\b${lang.toUpperCase()}`, 'u'), '').trim();
	}
