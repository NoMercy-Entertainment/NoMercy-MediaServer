...
Knex.QueryBuilder.extend('paginate', async function (perPage = 15, page = 1) {
	...
	return new Paginator(results, parseInt(total, 10), perPage, page);
});
...
class sutando {
	...
	static connection(connection: string|null = null) {
	...
	static commit(name: string|null = null) {
		//
	}

	static rollBack(name: string|null = null) {
		//
	}

	static schema(name: string|null = null) {
		const connection = this.connection(name);
