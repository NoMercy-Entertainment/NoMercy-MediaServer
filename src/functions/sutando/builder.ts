...
export default class Builder {
	...
	hasNested(relations, operator = '>=', count = 1, boolean = 'and', callback = null) {
		...
		return this.has(relations.shift(), doesntHave
			? '<'
			: '>=', 1, boolean, closure);
	}
	...
	withAggregate(relations, column, action = null) {
		...
		const columns = this.query._statements.filter(item => item.grouping == 'columns').map(item => item.value)
			.flat();
		...
		for (let name in parses) {
			...
			if (action) {
				...
				const wrappedColumn = column === '*'
					? column
					: relation.getRelated().qualifyColumn(hashedColumn);

				expression = action === 'exists'
					? wrappedColumn
					: `${action}(${wrappedColumn})`;
			} else {
				expression = column;
			}
	...
	prepareNestedWithRelationships(relations, prefix = '') {
		...
		for (const key in relations) {
			const value = relations[key];
			let attribute = key; let attributeSelectConstraint = value;
	...
	async destroy(ids) {
		...
		let count = 0;
		const models = await this.model.newModelQuery().whereIn(key, ids)
			.get();
