...
export default class BelongsToMany extends Relation {
	...
	async get(columns = ['*']) {
		...
		columns = builder.query?._statements?.find(item => item.grouping == 'columns')
			? []
			: columns;
...
BelongsToMany.prototype.sync = async function (ids, detaching = true) {
	...
	const current = results.length === 0
		? []
		: results.map(result => result.toData()).pluck(this.relatedPivotKey)
			.all()
			.map(i => String(i));
...
BelongsToMany.prototype.withPivot = function (columns) {
	this.pivotColumns = _.concat(
		this.pivotColumns, _.isArray(columns)
			? columns
