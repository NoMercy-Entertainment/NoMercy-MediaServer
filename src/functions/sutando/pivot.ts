...
import { getRelationName, getRelationMethod, getAttrMethod, getGetterMethod, getSetterMethod } from './utils';
import { RelationNotFoundError } from './errors';

class Model {
	primaryKey = 'id'; // protected
	builder = null; // protected
	table: string | null = null; // protected
	...
	fresh() {
		...
		return this.constructor.query().where(this.getKeyName(), this.getKey())
			.first();
	}
	...
	async refresh() {
		...
		const model = await this.constructor.query().where(this.getKeyName(), this.getKey())
			.first();
	...
	newPivot(parent, attributes, table, exists, using = null) {
		return using
			? using.constructor.fromRawAttributes(parent, attributes, table, exists)
	...
	related(relation) {
		if (typeof this[getRelationMethod(relation)] !== 'function') {
			...
			throw new RelationNotFoundError(message, this);
		}
	...
	belongsToMany(model, table = null, foreignPivotKey = null, relatedPivotKey = null, parentKey = null, relatedKey = null) {
		const query = model.query();
		const instance = new model();
		table = table || [this.constructor.name, instance.constructor.name].sort().join('_')
			.toLocaleLowerCase();
...
export default class Pivot extends Model {
	incrementing = false;
	guarded = [];
	pivotParent;
	foreignKey;
